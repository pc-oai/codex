...
- For local source builds, bump `LOCAL_BUILD_NUMBER` in `codex-rs/tui/src/version.rs`
  once per local change set so the footer version marker advances and running sessions
  that need reload are obvious at a glance.
