...
| --- | --- | --- |
| Composer clears, edit-last, queue controls | `clear_composer_draft`, `edit_last_message_from_command`, queued edit/discard/steer keymap actions | `ctrl_x_clears_composer_without_emitting_ops`, the edit-last preview/commit/cancel app tests, `alt_down_steers_most_recent_queued_message`, `alt_enter_steers_most_recent_queued_message_while_one_is_queued`, queued-message binding tests under `chatwidget/tests/composer_submission.rs`, private local build `v68` `Ctrl-E` smoke covering preview, `Esc` displaced-draft restoration, and rollback resubmit, `previous_message_edit_mode_uses_distinct_composer_accent`, remote `footer_mode_edit_last_message_snapshot`, local build `v71` pencil edit-preview smoke, and local build `v81` showing `/i` selected ahead of `/ide` before `Ctrl-X` cleared that draft |
