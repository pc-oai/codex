...
path used it until the compact configuring placeholder was restored. A live
follow-up then exposed that placeholder itself as unwanted startup flash, so
`5a7c58dda99b` removes the transient header cell entirely; the still-accepted
`compact_session_header` configuration is now compatibility-only. The
completion pass also refreshed keymap-picker coverage after it still showed
the displaced `Alt-R` raw-output default instead of the restored manual-rename
default.
...
| --- | --- | --- |
| Composer clears, edit-last, queue controls | `clear_composer_draft`, `edit_last_message_from_command`, queued edit/discard/steer keymap actions | `ctrl_x_clears_composer_without_emitting_ops`, the edit-last preview/commit/cancel/failure app tests including immediate edit-surface close after accepted submit in `319daa6ed213`, `alt_down_steers_most_recent_queued_message`, `alt_enter_steers_most_recent_queued_message_while_one_is_queued`, queued-message binding tests under `chatwidget/tests/composer_submission.rs`, private local build `v68` `Ctrl-E` smoke covering preview, `Esc` displaced-draft restoration, and rollback resubmit, `previous_message_edit_mode_uses_distinct_composer_accent`, remote `footer_mode_edit_last_message_snapshot`, local build `v71` pencil edit-preview smoke, local build `v81` showing `/i` selected ahead of `/ide` before `Ctrl-X` cleared that draft, and clean detached local build `v66` showing `✎ Editing previous message` before Enter then normal `Working` state immediately after the edited submit |
...
| Transcript and startup chrome | `ToggleCondensedTranscriptView`, async fresh `InitialThreadStarted` startup handoff, bundled `local_bootstrap_models` with deferred `ModelsLoaded` refresh, no initial session-header active cell, startup skills refresh, right-side footer/MCP status | startup waiting-gate tests, `enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit`, `local_bootstrap_models_loads_bundled_catalog_with_default`, `startup_does_not_render_transient_session_header_table` plus its no-table initial-frame snapshot, earlier live `v81` evidence identifying the unwanted compact-placeholder flash, clean detached local build `v66` first paint showing only prompt/footer while MCP progress stayed in the footer, source TUI `v79` first-paint smoke showing `codex | Ready` before the fresh thread title gained suffix `…8fbcf5c9`, clean source TUI `v65` startup showing a concrete `hoothoot default` footer model before configured thread attach, condensed reflow and restored scrollback tests, clear-header snapshots, status/footer snapshots, and the `Alt-C` terminal smoke already done for archived local build `v64` |
...
The latest follow-up also ran isolated remote focused checks for
`edit_last_message` (eight passing tests) and
`startup_does_not_render_transient_session_header_table` (passing). The latter
stream included the already-recorded stale app-server/protocol artifact noise
from concurrent remote work, while the targeted TUI test process exited
successfully.
