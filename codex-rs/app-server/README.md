...
- `thread/spawn` — spawn a child agent from an already loaded parent thread and start it with the supplied `input`; the new child stays in the parent session tree, emits `thread/started`, and can be reached by the parent agent later through the collab tools. Pass `taskName` to choose the child agent path segment, or omit it to let the server assign one.
...
`thread/spawn` creates a thread-spawn descendant, not a separate root thread or an ephemeral fork. The child inherits the parent thread's current model and runtime permissions. If `taskName` is omitted, app-server assigns a unique path segment for collab routing.
