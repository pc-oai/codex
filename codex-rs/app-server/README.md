...
`thread/spawn` creates a thread-spawn descendant, not a separate root thread or an ephemeral fork. The child inherits the parent thread's current model and runtime permissions. If `input` is empty, the child starts idle. If `taskName` is omitted, app-server assigns a unique path segment for collab routing.
