...
- `thread/spawn` — spawn a child agent from an already loaded parent thread. The child starts from sanitized completed parent turns by default; if the parent has a turn in progress, that unfinished turn is omitted. Pass `history: "fresh"` to start with no parent turns. Non-empty `input` starts the child immediately; empty `input` leaves it idle for a later `turn/start`. The new child stays in the parent session tree, emits `thread/started`, and can be reached by the parent agent later through the collab tools. Pass `taskName` to choose the child agent path segment, or omit it to let the server assign one.
...
- `thread/list` — page through stored rollouts; supports cursor-based pagination and optional `modelProviders`, `sourceKinds`, `archived`, `cwd`, `searchTerm`, and `userStates` filters. Each returned `thread` includes `status` (`ThreadStatus`), defaulting to `notLoaded` when the thread is not currently loaded.
...
- `thread/metadata/update` — patch stored thread metadata in sqlite; currently supports updating persisted `gitInfo` fields or `userState` (`active`, `done`, or `parked`) and returns the refreshed `thread`.
...
- `thread/close` — close a loaded spawned child agent thread and any live spawned descendants while marking that child path closed for future subagent-tree resume; returns `{}` on success.
- `thread/delete` — permanently remove a thread’s persisted rollout file and sqlite metadata; returns `{}` on success.
...
To start a child agent without waiting for the parent thread to call the collab `spawn_agent` tool, call `thread/spawn` on a loaded parent thread:

```json
{ "method": "thread/spawn", "id": 11, "params": {
    "threadId": "thr_123",
    "input": [{ "type": "text", "text": "Check the parser tests.", "textElements": [] }]
} }
{ "id": 11, "result": { "thread": { "id": "thr_child", "sessionId": "thr_123", … } } }
{ "method": "thread/started", "params": { "thread": { … } } }
```

`thread/spawn` creates a thread-spawn descendant, not a separate root thread or an ephemeral fork. The child inherits the parent thread's current model and runtime permissions. By default it copies replayable completed turns from its parent history into the response and sets `thread.forkedFromId` to the parent thread; a parent turn that is still in progress is excluded. Pass `history: "fresh"` to keep the child in the same session tree with an empty transcript instead. If `input` is empty, the child starts idle. If `taskName` is omitted, app-server assigns a unique path segment for collab routing.
...
Use `thread/metadata/update` to patch sqlite-backed metadata for a thread without resuming it. Today this supports either persisted `gitInfo` or the thread `userState`; omitted Git fields are left unchanged, while explicit `null` clears a stored Git value.
