...
#[tokio::test]
async fn thread_metadata_update_sets_user_state_and_thread_read_preserves_it() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    let update_id = mcp
        .send_thread_metadata_update_request(ThreadMetadataUpdateParams {
            thread_id: thread.id.clone(),
            user_state: Some(ThreadUserState::Parked),
            git_info: None,
        })
        .await?;
    let update_resp: JSONRPCResponse = timeout(
    ...
    let ThreadMetadataUpdateResponse { thread: updated } =
        to_response::<ThreadMetadataUpdateResponse>(update_resp)?;
    assert_eq!(updated.user_state, ThreadUserState::Parked);
    ...
    .await??;
    let ThreadReadResponse { thread: read, .. } = to_response::<ThreadReadResponse>(read_resp)?;
    assert_eq!(read.user_state, ThreadUserState::Parked);
