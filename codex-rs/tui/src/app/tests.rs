...
fn plain_rendered_line_text(line: &Line<'_>) -> String {
    line.to_string()
}
...
async fn redraw_full_scrollback_ignores_automatic_resize_row_cap() {
    ...
    assert_eq!(plain_rendered_line_text(&rendered[0]), "cell 0");
    assert_eq!(plain_rendered_line_text(&rendered[6]), "cell 3");
}
