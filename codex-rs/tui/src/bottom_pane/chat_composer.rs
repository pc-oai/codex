...
use self::footer_state::FooterFlash;
...
impl ChatComposer {
    ...
    pub(crate) fn show_agent_navigation_strip(&mut self, line: Line<'static>, duration: Duration) {
        ...
        self.footer.agent_navigation_strip = Some(FooterFlash { line, expires_at });
    }

    fn active_agent_context_line(&self) -> Option<Line<'static>> {
        let base = self
            .footer
            .active_agent_label
            .as_ref()
    ...
    fn slash_popup_btw_for_bt_logic() {
        ...
        match &composer.popups.active {
            ActivePopup::Command(popup) => match popup.selected_item() {
                ...
                Some(CommandItem::BuiltinAlias { name, .. }) => {
                    panic!("expected btw command, got alias {name}")
                }
                Some(CommandItem::ServiceTier(command)) => {
    ...
    fn slash_popup_side_for_si_logic() {
        ...
        match &composer.popups.active {
            ActivePopup::Command(popup) => match popup.selected_item() {
                ...
                Some(CommandItem::BuiltinAlias { name, .. }) => {
                    panic!("expected side command, got alias {name}")
                }
                Some(CommandItem::ServiceTier(command)) => {
