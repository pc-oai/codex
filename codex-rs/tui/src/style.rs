...
pub fn edited_user_message_style() -> Style {
    edited_user_message_style_for(default_bg())
}
...
pub fn edited_user_message_style_for(terminal_bg: Option<(u8, u8, u8)>) -> Style {
    match terminal_bg {
        Some(bg) => Style::default().bg(edited_user_message_bg(bg)),
        None => Style::default(),
    }
}
...
#[allow(clippy::disallowed_methods)]
pub fn edited_user_message_bg(terminal_bg: (u8, u8, u8)) -> Color {
    let (top, alpha) = if is_light(terminal_bg) {
        ((180, 110, 0), 0.08)
    } else {
        ((255, 196, 0), 0.18)
